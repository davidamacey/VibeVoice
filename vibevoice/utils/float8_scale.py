"""
Float8 E4M3FN aware layer wrappers for inference.

These wrappers cast weights/biases to the input dtype before each forward pass,
enabling inference with float8_e4m3fn quantized models.

Referenced from ComfyUI (https://github.com/comfyanonymous/ComfyUI).
"""
...
from vibevoice.utils.vibevoice_norm import QwenRMSNorm


def cast_bias_weight(module, input: torch.Tensor = None, dtype: torch.dtype = None,
                     device: torch.device = None, bias_dtype: torch.dtype = None):
    if input is None:
    ...
    class QwenRMSNorm(QwenRMSNorm, ResetParametersMixin):
        ...
        def forward_comfy_cast_weights(self, input):
            if self.weight is not None:
                ...
                weight = None
            return super().forward(input, weight)
